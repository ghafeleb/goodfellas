...
class DeCLUTRDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        labels: Iterable[Any] = []
        my_shuffle = lambda: 0.1
        #with open('train_labels.csv','r') as label_file:
             #labels = list(enumerate(label_file))
             #random.shuffle(labels,my_shuffle)
             #labels = iter(labels)
        with open(file_path, "r") as data_file:
            ...
            #for text,label in zip(data,labels):
                #yield self.text_to_instance(text[1].strip(),label[1].strip())
            for _,line in data:
                line = line.strip("\n")
                if not line:
                    continue
                line_json = json.loads(line)
                yield self.text_to_instance(line_json["text"],line_json["label"])
