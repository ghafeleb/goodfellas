...
class PyTorchMetricLearningLoss(Registrable):
    ...
    def get_embeddings_and_labels(
            self, anchors_0: torch.Tensor, positives_0: torch.Tensor, anchors_1: torch.Tensor, positives_1: torch.Tensor
        ...
        loss function.
        """
        embeddings_0 = torch.cat((anchors_0, positives_0))
        indices_0 = torch.randint(sys.maxsize, (anchors_0.size(0),), device=anchors_0.device)
        labels_0 = torch.cat((indices_0, indices_0))

        embeddings_1 = torch.cat((anchors_1, positives_1))
        indices_1 = torch.randint(sys.maxsize, (anchors_1.size(0),), device=anchors_1.device)
        labels_1 = torch.cat((indices_1, indices_1))

        embeddings = torch.cat((embeddings_0, embeddings_1))
        labels = torch.cat((labels_0, labels_1))

        parties = torch.cat((torch.zeros(embeddings_0.size(0)), torch.ones(embeddings_1.size(0)))) # todo the second one should be ones
        parties = parties.to(device=anchors_0.device)


        return embeddings, labels, parties
